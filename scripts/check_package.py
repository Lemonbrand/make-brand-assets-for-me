...
def check_package(root):
    ...
    required = [
        ...
        "plugins/make-brand-assets-for-me/skills/brand-assets-make-scene/scripts/compose_text_overlay.py",
        "plugins/make-brand-assets-for-me/skills/brand-assets-make-scene/scripts/check_background_fit.py",
        "plugins/make-brand-assets-for-me/skills/brand-assets-make-launch-pack/scripts/compose_text_overlay.py",
        "plugins/make-brand-assets-for-me/skills/brand-assets-make-launch-pack/scripts/check_background_fit.py",
    ...
    if found != SKILLS:
        errors.append(f"Expected seven skills, found: {', '.join(sorted(found))}")
    for name in SKILLS:
    ...
    if public_found != SKILLS:
        errors.append(f"Expected seven public skills, found: {', '.join(sorted(public_found))}")
...
def main():
    ...
    if errors:
        for error in errors:
            print(f"FAIL: {error}")
        raise SystemExit(1)
    print("PASS: the public package has seven synchronized skills, two plugin adapters, ten baselines, a complete launch proof, and its required files.")
